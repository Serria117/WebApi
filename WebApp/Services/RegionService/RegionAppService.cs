...
{
    Task<ResponseEntity> CreateProvinceAsync(ProvinceCreateDto input);
    Task<ResponseEntity> CreateDistrictAsync(DistrictCreateDto input);
    Task<ResponseEntity> CreateTaxOfficeAsync(TaxOfficeCreateDto input);
    Task<ResponseEntity> GetAllProvincesAsync(PageRequest page);
    Task<ResponseEntity> GetProvinceAsync(int id);
    Task<ResponseEntity> GetDistrictsInProvinceAsync(int provinceId);
    Task<ResponseEntity> GetTaxOfficesInProvinceAsync(int provinceId);
    Task<ResponseEntity> CreateManyProvincesAsync(List<ProvinceCreateDto> input);
    Task<ResponseEntity> CreateManyDistrictsAsync(int provinceId, List<DistrictCreateDto> input);
    Task<ResponseEntity> GetTaxOfficesByParentAsync(int parentId);
    Task<ResponseEntity> CreateManyTaxOfficeAsync(int pId, List<TaxOfficeCreateDto> input);
    Task<ResponseEntity> UpdateProvinceAsync(int id, ProvinceCreateDto input);
    Task<ResponseEntity> GetAllTaxOfficesAsync(PageRequest req);
    Task<ResponseEntity> FindTaxOfficeByIdAsync(int id);
    Task<ResponseEntity> FindTopLevelTaxOfficesAsync();
    Task<ResponseEntity> UpdateTaxOfficeAsync(int id, TaxOfficeCreateDto input);
    Task<bool> TaxOfficeCodeExists(string code);
    Task<ResponseEntity> GetDistrictById(int id);
    Task<ResponseEntity> CreateTaxOffice2(List<TaxOffice2CreateDto> dtos);
    Task<ResponseEntity> GetTaxOffice2ByProvince(int provinceId);
    Task<ResponseEntity> GetTaxOffice2ByDistrict(string districtCode, int provinceId);
    Task<ResponseEntity> GetAllTaxOffice2(PageRequest req);
    Task<ResponseEntity> GetTaxOffice2ById(int id);
    Task<ResponseEntity> UpdateTaxOffice2(int id, TaxOffice2CreateDto input);
}
...
{
    public async Task<ResponseEntity> CreateProvinceAsync(ProvinceCreateDto input)
    {
    ...
    public async Task<ResponseEntity> CreateManyProvincesAsync(List<ProvinceCreateDto> input)
    {
        try
        {
        ...
        catch (Exception e)
        {
            ...
            return ResponseEntity.Error("Failed to create provinces");
        }
    }

    public async Task<ResponseEntity> UpdateProvinceAsync(int id, ProvinceCreateDto input)
    {
        try
        {
    ...
    public async Task<ResponseEntity> CreateDistrictAsync(DistrictCreateDto input)
    {
        if (!await provinceRepo.ExistAsync(x => x.Id == input.ProvinceId))
            return ResponseEntity.Error("Province could not be found");
        var district = input.ToEntity(provinceRepo);
        var saved = await districtRepo.CreateAsync(district);
        return ResponseEntity.OkResult(saved.ToDisplayDto());
    }

    public async Task<ResponseEntity> CreateManyDistrictsAsync(int provinceId, List<DistrictCreateDto> input)
    {
        try
        {
            ...
            return ResponseEntity.OkResult("OK");
        }
        catch (Exception e)
        {
            logger.LogError("Error: {message}", e.Message);
            return ResponseEntity.Error("Failed to create districts");
        }
    }

    public async Task<ResponseEntity> GetDistrictById(int id)
    {
        var district = await districtRepo.Find(x => x.Id == id && !x.Deleted).FirstOrDefaultAsync();
        if (district is null)
        {
            return ResponseEntity.Error404("District not found");
        }
        return ResponseEntity.OkResult(district.ToDisplayDto());
    }

    public async Task<ResponseEntity> CreateTaxOfficeAsync(TaxOfficeCreateDto input)
    {
        if (await TaxOfficeCodeExists(input.Code))
            return ResponseEntity.Error400($"Tax Office Code '{input.Code}' already exists");

        if (input.ProvinceId is not null && !await provinceRepo.ExistAsync(x => x.Id == input.ProvinceId && !x.Deleted))
            return ResponseEntity.Error400("Province could not be found");

        // check for parent existence
        if (input.ParentId is not null && !await taxRepo.ExistAsync(x => x.Id == input.ParentId && !x.Deleted))
            return ResponseEntity.Error400("Parent Tax Office could not be found");
    ...
    public async Task<ResponseEntity> UpdateTaxOfficeAsync(int id, TaxOfficeCreateDto input)
    {
        try
        {
        ...
        {
            return ResponseEntity.Error400(e.Message);
        }
        catch (Exception e)
        {
    ...
    public async Task<ResponseEntity> CreateManyTaxOfficeAsync(int pId, List<TaxOfficeCreateDto> input)
    {
        try
        {
        ...
        catch (Exception e)
        {
            ...
            return ResponseEntity.Error("Failed to create tax offices");
        }
    }
    ...
    public async Task<ResponseEntity> GetAllTaxOfficesAsync(PageRequest req)
    {
        ...
        return ResponseEntity.OkResult(parents.MapPagedList(x => x.ToDisplayDto()));
    }

    public async Task<ResponseEntity> FindTaxOfficeByIdAsync(int id)
    {
        ...
        {
            return ResponseEntity.Error404("Tax Office could not be found");
        }
        ...
        return ResponseEntity.OkResult(taxOffice.ToDisplayDto());
    }

    public async Task<ResponseEntity> FindTopLevelTaxOfficesAsync()
    {
        var topLevelTaxOffices = await taxRepo.Find(x => x.ParentId == null || x.ParentId.Value == 0).ToListAsync();
        return ResponseEntity.OkResult(topLevelTaxOffices.MapCollection(x => x.ToDisplayDto()));
    }

    public async Task<ResponseEntity> GetAllProvincesAsync(PageRequest page)
    {
        ...
        return ResponseEntity.OkResult(result.MapPagedList(x => x.ToDisplayDto()));
    }

    public async Task<ResponseEntity> GetProvinceAsync(int id)
    {
        ...
        return province == null
            ? ResponseEntity.Error404("Province could not be found")
            : ResponseEntity.OkResult(province.ToDisplayDto());
    }

    public async Task<ResponseEntity> GetDistrictsInProvinceAsync(int provinceId)
    {
        var districts = await districtRepo.Find(filter: d => d.Province!.Id == provinceId && !d.Deleted,
                                                sortBy: "Code", order: "ASC")
                                          .ToListAsync();
        if (districts.Count == 0) return ResponseEntity.Error404("No districts found for the given province ID");
        return new ResponseEntity
        {
    ...
    public async Task<ResponseEntity> GetTaxOfficesInProvinceAsync(int provinceId)
    {
        var taxOffices = await taxRepo.Find(filter: t => t.Province != null && t.Province.Id == provinceId,
                                            sortBy: "Id", order: "ASC")
                                      .AsNoTracking()
                                      .ToListAsync();
        return ResponseEntity.OkResult(taxOffices.MapCollection(x => x.ToDisplayDto()));
    }

    public async Task<ResponseEntity> GetTaxOfficesByParentAsync(int parentId)
    {
    ...
    public async Task<ResponseEntity> CreateTaxOffice2(List<TaxOffice2CreateDto> dtos)
    {
        ...
        {
            return ResponseEntity.Error400("No valid tax office data");
        }
    ...
    public async Task<ResponseEntity> GetTaxOffice2ByProvince(int provinceId)
    {
        var taxOffices = await taxRepo2.Find(x => x.ProvinceId == provinceId && !x.Deleted)
                                       .ToListAsync();
        return ResponseEntity.OkResult(taxOffices.Select(x => new TaxOffice2DiplayDto
        {
            Id = x.Id,
            FullName = x.FullName,
    ...
    public async Task<ResponseEntity> GetTaxOffice2ByDistrict(string districtCode, int provinceId)
    {
    ...
    public async Task<ResponseEntity> GetAllTaxOffice2(PageRequest req)
    {
        ...
        return ResponseEntity.OkResult(taxOffices.MapPagedList(x => new TaxOffice2DiplayDto
        {
            Id = x.Id,
            FullName = x.FullName,
            ShortName = x.ShortName,
    ...
    public async Task<ResponseEntity> GetTaxOffice2ById(int id)
    {
        var found = await taxRepo2.Find(x => x.Id == id && !x.Deleted)
                                  .FirstOrDefaultAsync();
        if (found is null)
        {
            return ResponseEntity.Error404("Not found");
        }
        return ResponseEntity.OkResult(new TaxOffice2DiplayDto
        {
            Id = found.Id,
    ...
    public async Task<ResponseEntity> UpdateTaxOffice2(int id, TaxOffice2CreateDto input)
    {
        try
        {
